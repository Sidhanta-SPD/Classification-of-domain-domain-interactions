...
def interfaceResidues_area(cmpx, cA='c. A', cB='c. B', cutoff=1.0, selName="interface"):
	"""
	...
	cmd.set("dot_solvent", 1)#setting this will calculate SASA instead of molecular surface area
	...
	ar=cmd.get_area(selName)
	ar_format="{:.2f}".format(ar)
	# reset users settings
	cmd.set("dot_solvent", oldDS)#resetting the dot solvent
...
cmd.extend("interfaceResidues_area", interfaceResidues_area)

#cmd.load('1aoa_edit.pdb','3jz3')
#cmd.disable('all')
#cmd.enable('3jz3')
#print(interfaceResidues_area("3jz3", cA="c. A", cB="c. B", cutoff=1, selName="foundIn3jz3"))
#it uses pymol script to find interface area of two chains. in A^square
